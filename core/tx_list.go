...
// Remove deletes a transaction from the maintained map, returning whetzer the
...
// prevent getting into and invalid state. This is not sometzing that should ever
...
type txList struct {
	strict bool         // Whetzer nonces are strictly continuous or not
...
// Overlaps returns whetzer the transaction specified has the same nonce as one
// already contained within the list.
func (l *txList) Overlaps(tx *types.Transaction) bool {
	return l.txs.Get(tx.Nonce()) != nil
}

// Add tries to insert a new transaction into the list, returning whetzer the
...
// Remove deletes a transaction from the maintained list, returning whetzer the
...
// Empty returns whetzer the list of transactions is empty or not.
...
// Underpriced checks whetzer a transaction is cheaper than (or as cheap as) the
