...
var (
	...
	configPrefix   = []byte("ethzero-config-") // config prefix for the db
...
func WriteCanonicalHash(db etzdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db etzdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db etzdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db etzdb.Putter, hash common.Hash) error {
...
func WriteHeader(db etzdb.Putter, header *types.Header) error {
...
func WriteBody(db etzdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db etzdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db etzdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db etzdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db etzdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db etzdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db etzdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db etzdb.Database) etzdb.Database {
	return etzdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db etzdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db etzdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db etzdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
