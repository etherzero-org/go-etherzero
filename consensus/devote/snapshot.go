...
func (snap *Snapshot) election(genesis, first, parent *types.Header, nodes []string, safeSize int, maxWitnessSize uint64) ([]string, error) {
	...
	for i := prevCycle; i < currentCycle; i++ {
		...
		votes, err := snap.masternodes(parent, preisgenesis, list)
		if err != nil {
