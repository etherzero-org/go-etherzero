...
func (ethash *Ethash) Seal(chain consensus.ChainReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) (*types.Block, error) {
	...
	go func() {
		...
		case <-ethash.update:
			...
			close(abort)
			if _,err := ethash.Seal(chain, block, results, stop); err != nil {
