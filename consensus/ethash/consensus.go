...
func accumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, uncles []*types.Header, masternode common.Address) {
	...
	//
	if config.IsEthzeroMasternode(header.Number) {
		...
		state.AddBalance(params.MainnetGovernanceAddress, blockReward)
		log.Trace("Masternode Reward ", "account ", masternode, "GovernanceAddress", GovernanceReward, "blockReward", blockReward)
	}
