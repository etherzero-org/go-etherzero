...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-etz/master/doc/etzapp.asc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and Ethzero address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an Ethzero transaction after having the user validate the parameters
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Ethzero app in browser mode", w.failure
	}
	if w.offline() {
		return "Ethzero app offline", w.failure
	}
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// Ethzero app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the Ethzero app's version, will fail prior to v1.0.2
...
// and returning the Ethzero address located on that derivation path.
...
//
// Note, if the version of the Ethzero application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested nonetzeless, an error
// will be returned opposed to silently signing in Homestead mode.
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the Ethzero app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the Ethzero wallet app running
...
// ledgerDerive retrieves the currently active Ethzero address from a Ledger
...
//   Ethzero address length | 1 byte
//   Ethzero address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the Ethzero hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an Ethzero address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Extract the Ethzero signature and do a sanity validation
	if len(reply) != 65 {
