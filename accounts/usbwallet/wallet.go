...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Ethzero
...
type wallet struct {
	...
	deriveChain    ethzero.ChainStateReader // Blockchain state reader to discover used account with
...
// whetzer they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Ethzero account
			...
			// If the next account is empty, stop self-derivation, but add it nonetzeless
...
// Contains implements accounts.Wallet, returning whetzer a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain ethzero.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
