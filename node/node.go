...
func (n *Node) OpenDatabase(name string, cache, handles int) (etzdb.Database, error) {
	if n.config.DataDir == "" {
		return etzdb.NewMemDatabase()
	}
	return etzdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
