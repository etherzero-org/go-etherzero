...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (etzdb.Database, error) {
	if ctx.config.DataDir == "" {
		return etzdb.NewMemDatabase()
	}
	db, err := etzdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
