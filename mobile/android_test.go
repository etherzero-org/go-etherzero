...
import (
	...
	"github.com/ethzero/go-ethzero/internal/build"
)
...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Getz.LightScryptN, Getz.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.ethzero", new NodeConfig());
			node.start();
			...
			EthzeroClient ec = node.getEthzeroClient();
	...
	// signatures too. Regression test for go-ethzero issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogetzer
	...
	workspace, err := ioutil.TempDir("", "getz-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/ethzero/getztest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Getz and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethzero", "github.com/ethzero/go-ethzero/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "getz.aar"), "getz.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "ethzero", "getztest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
