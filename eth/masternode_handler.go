...
func NewMasternodeManager(config *params.ChainConfig, mode downloader.SyncMode, networkId uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb ethdb.Database) (*MasternodeManager, error) {
	...
	for i, version := range MasternodeProtocolVersions {
		...
		if mode == downloader.FastSync && version < etz64 {
			continue
		}
...
func (mm *MasternodeManager) removePeer(id string) {
	...
	mm.downloader.UnregisterPeer(id)
...
func (mm *MasternodeManager) handle(p *peer) error {
	// Ignore maxPeers if this is a trusted peer
	if mm.peers.Len() >= mm.maxPeers && !p.Peer.Info().Network.Trusted {
		return p2p.DiscTooManyPeers
	}
	p.Log().Debug("Etherzero masternode peer connected", "name", p.Name())

	// Execute the Ethereum handshake
	var (
		genesis = mm.blockchain.Genesis()
		head    = mm.blockchain.CurrentHeader()
		hash    = head.Hash()
		number  = head.Number.Uint64()
		td      = mm.blockchain.GetTd(hash, number)
	)
	if err := p.Handshake(mm.networkId, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Etherzero masternode handshake failed", "err", err)
		return err
	}
	...
	if err := mm.peers.Register(p); err != nil {
		p.Log().Error("Etherzero masternode peer registration failed", "err", err)
		return err
	}
	defer mm.removePeer(p.id)

	// Register the peer in the downloader. If the downloader considers it banned, we disconnect
	if err := mm.downloader.RegisterPeer(p.id, p.version, p); err != nil {
		return err
	}
	// Propagate existing transactions. new transactions appearing
	// after this will be sent via broadcasts.
	mm.syncTransactions(p)

	// If we're DAO hard-fork aware, validate any remote peer with regard to the hard-fork
	if daoBlock := mm.chainconfig.DAOForkBlock; daoBlock != nil {
	...
	for {
		if err := mm.handleMsg(p); err != nil {
			p.Log().Debug("Etherzero masternode message handling failed", "err", err)
			return err
		}
	}
}
...
func (mm *MasternodeManager) handleMsg(p *peer) error {
	...
	case msg.Code == GetBlockHeadersMsg:
		...
		for !unknown && len(headers) < int(query.Amount) && bytes < softResponseLimit && len(headers) < downloader.MaxHeaderFetch {
			...
			if hashMode {
				origin = mm.blockchain.GetHeaderByHash(query.Origin.Hash)
			} else {
				origin = mm.blockchain.GetHeaderByNumber(query.Origin.Number)
			}
			if origin == nil {
				break
			}
			...
			case query.Origin.Hash != (common.Hash{}) && query.Reverse:
				...
				for i := 0; i < int(query.Skip)+1; i++ {
					if header := mm.blockchain.GetHeader(query.Origin.Hash, number); header != nil {
			...
			case query.Origin.Hash != (common.Hash{}) && !query.Reverse:
				...
				if next <= current {
					...
					if header := mm.blockchain.GetHeaderByNumber(next); header != nil {
						if mm.blockchain.GetBlockHashesFromHash(header.Hash(), query.Skip+1)[query.Skip] == query.Origin.Hash {
	...
	case msg.Code == BlockHeadersMsg:
		...
		if len(headers) == 0 && p.forkDrop != nil {
			...
			if daoHeader := mm.blockchain.GetHeaderByNumber(mm.chainconfig.DAOForkBlock.Uint64()); daoHeader != nil {
				if _, td := p.Head(); td.Cmp(mm.blockchain.GetTd(daoHeader.Hash(), daoHeader.Number.Uint64())) >= 0 {
					verifyDAO = false
				}
			}
		...
		if filter {
			...
			if p.forkDrop != nil && mm.chainconfig.DAOForkBlock.Cmp(headers[0].Number) == 0 {
				// Disable the fork drop timer
				p.forkDrop.Stop()
				p.forkDrop = nil

				// Validate the header and either drop the peer or continue
				if err := misc.VerifyDAOHeaderExtraData(mm.chainconfig, headers[0]); err != nil {
			...
			headers = mm.fetcher.FilterHeaders(p.id, headers, time.Now())
		}
		if len(headers) > 0 || !filter {
			err := mm.downloader.DeliverHeaders(p.id, headers)
			if err != nil {
	...
	case msg.Code == GetBlockBodiesMsg:
		...
		for bytes < softResponseLimit && len(bodies) < downloader.MaxBlockFetch {
			...
			if data := mm.blockchain.GetBodyRLP(hash); len(data) != 0 {
	...
	case msg.Code == BlockBodiesMsg:
		...
		if filter {
			trasactions, uncles = mm.fetcher.FilterBodies(p.id, trasactions, uncles, time.Now())
		}
		if len(trasactions) > 0 || len(uncles) > 0 || !filter {
			err := mm.downloader.DeliverBodies(p.id, trasactions, uncles)
			if err != nil {
	...
	case p.version >= etz64 && msg.Code == NodeDataMsg:
		...
		if err := mm.downloader.DeliverNodeData(p.id, data); err != nil {
	...
	case p.version >= etz64 && msg.Code == ReceiptsMsg:
	...
	case msg.Code == NewBlockHashesMsg:
		...
		for _, block := range announces {
			if !mm.blockchain.HasBlock(block.Hash, block.Number) {
				unknown = append(unknown, block)
			}
		}
		for _, block := range unknown {
			mm.fetcher.Notify(p.id, block.Hash, block.Number, time.Now(), p.RequestOneHeader, p.RequestBodies)
		}
	...
	case msg.Code == NewBlockMsg:
		...
		mm.fetcher.Enqueue(p.id, request.Block)
		...
		if _, td := p.Head(); trueTD.Cmp(td) > 0 {
			...
			if trueTD.Cmp(mm.blockchain.GetTd(currentBlock.Hash(), currentBlock.NumberU64())) > 0 {
				go mm.synchronise(p)
			}
		}
...
// MasternodeInfo retrieves some masternode protocol metadata about the running host node.
func (self *MasternodeManager) MasternodeInfo() *MasternodeInfo {
