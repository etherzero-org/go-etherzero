...
import (
	...
	"github.com/ethzero/go-ethzero/logger"
	"github.com/ethzero/go-ethzero/p2p"
	"github.com/ethzero/go-ethzero/p2p/nat"
	"github.com/ethzero/go-ethzero/whisper"
)
...
func main() {
	...
	// Create an Ethzero peer to communicate through
	server := p2p.Server{
		PrivateKey: key,
		MaxPeers:   10,
		Name:       name,
	...
	fmt.Println("Starting Ethzero peer...")
	if err := server.Start(); err != nil {
		fmt.Printf("Failed to start Ethzero peer: %v.\n", err)
		os.Exit(1)
	}

	// Send a message to self to check that sometzing works
