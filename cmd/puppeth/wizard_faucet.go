...
func (w *wizard) deployFaucet() {
	...
	if err != nil {
		infos = &faucetInfos{
			node:    &nodeInfos{portFull: 21212, peersTotal: 25},
			port:    80,
			host:    client.server,
			amount:  1,
			minutes: 1440,
	...
	fmt.Println()
	if infos.node.etzstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.etzstats = w.readString() + ":" + w.conf.etzstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.etzstats)
		infos.node.etzstats = w.readDefaultString(infos.node.etzstats) + ":" + w.conf.etzstats
	}
