...
// cppEthzeroGenesisSpec represents the genesis specification format used by the
// C++ Ethzero implementation.
type cppEthzeroGenesisSpec struct {
	...
	Accounts map[common.Address]*cppEthzeroGenesisSpecAccount `json:"accounts"`
}

// cppEthzeroGenesisSpecAccount is the prefunded genesis account and/or precompiled
// contract definition.
type cppEthzeroGenesisSpecAccount struct {
	Balance     *hexutil.Big                   `json:"balance"`
	Nonce       uint64                         `json:"nonce,omitempty"`
	Precompiled *cppEthzeroGenesisSpecBuiltin `json:"precompiled,omitempty"`
}

// cppEthzeroGenesisSpecBuiltin is the precompiled contract definition.
type cppEthzeroGenesisSpecBuiltin struct {
	Name          string                               `json:"name,omitempty"`
	StartingBlock hexutil.Uint64                       `json:"startingBlock,omitempty"`
	Linear        *cppEthzeroGenesisSpecLinearPricing `json:"linear,omitempty"`
}

type cppEthzeroGenesisSpecLinearPricing struct {
	Base uint64 `json:"base"`
	Word uint64 `json:"word"`
}

// newCppEthzeroGenesisSpec converts a go-ethzero genesis block into a Parity specific
// chain specification format.
func newCppEthzeroGenesisSpec(network string, genesis *core.Genesis) (*cppEthzeroGenesisSpec, error) {
	// Only ethash is currently supported between go-ethzero and cpp-ethzero
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in Parity's format
	spec := &cppEthzeroGenesisSpec{
		SealEngine: "Ethash",
	}
	...
	spec.Accounts = make(map[common.Address]*cppEthzeroGenesisSpecAccount)
	for address, account := range genesis.Alloc {
		spec.Accounts[address] = &cppEthzeroGenesisSpecAccount{
			Balance: (*hexutil.Big)(account.Balance),
			Nonce:   account.Nonce,
		}
	}
	spec.Accounts[common.BytesToAddress([]byte{1})].Precompiled = &cppEthzeroGenesisSpecBuiltin{
		Name: "ecrecover", Linear: &cppEthzeroGenesisSpecLinearPricing{Base: 3000},
	}
	spec.Accounts[common.BytesToAddress([]byte{2})].Precompiled = &cppEthzeroGenesisSpecBuiltin{
		Name: "sha256", Linear: &cppEthzeroGenesisSpecLinearPricing{Base: 60, Word: 12},
	}
	spec.Accounts[common.BytesToAddress([]byte{3})].Precompiled = &cppEthzeroGenesisSpecBuiltin{
		Name: "ripemd160", Linear: &cppEthzeroGenesisSpecLinearPricing{Base: 600, Word: 120},
	}
	spec.Accounts[common.BytesToAddress([]byte{4})].Precompiled = &cppEthzeroGenesisSpecBuiltin{
		Name: "identity", Linear: &cppEthzeroGenesisSpecLinearPricing{Base: 15, Word: 3},
	}
	if genesis.Config.ByzantiumBlock != nil {
		spec.Accounts[common.BytesToAddress([]byte{5})].Precompiled = &cppEthzeroGenesisSpecBuiltin{
			Name: "modexp", StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64()),
		}
		spec.Accounts[common.BytesToAddress([]byte{6})].Precompiled = &cppEthzeroGenesisSpecBuiltin{
			Name: "alt_bn128_G1_add", StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64()), Linear: &cppEthzeroGenesisSpecLinearPricing{Base: 500},
		}
		spec.Accounts[common.BytesToAddress([]byte{7})].Precompiled = &cppEthzeroGenesisSpecBuiltin{
			Name: "alt_bn128_G1_mul", StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64()), Linear: &cppEthzeroGenesisSpecLinearPricing{Base: 40000},
		}
		spec.Accounts[common.BytesToAddress([]byte{8})].Precompiled = &cppEthzeroGenesisSpecBuiltin{
...
// newParityChainSpec converts a go-ethzero genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only ethash is currently supported between go-ethzero and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in Parity's format
	...
	spec.Genesis.Seal.Ethzero.Nonce = (hexutil.Bytes)(make([]byte, 8))
	binary.LittleEndian.PutUint64(spec.Genesis.Seal.Ethzero.Nonce[:], genesis.Nonce)

	spec.Genesis.Seal.Ethzero.MixHash = (hexutil.Bytes)(genesis.Mixhash[:])
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
// pyEthzeroGenesisSpec represents the genesis specification format used by the
// Python Ethzero implementation.
type pyEthzeroGenesisSpec struct {
	Nonce      hexutil.Bytes     `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	GasLimit   hexutil.Uint64    `json:"gasLimit"`
...
// newPyEthzeroGenesisSpec converts a go-ethzero genesis block into a Parity specific
// chain specification format.
func newPyEthzeroGenesisSpec(network string, genesis *core.Genesis) (*pyEthzeroGenesisSpec, error) {
	// Only ethash is currently supported between go-ethzero and pyethzero
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	spec := &pyEthzeroGenesisSpec{
