...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for etzstats availability")
	if infos, err := checkEthstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["etzstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["etzstats"] = infos.Report()
		etzstats = infos.config
	}
