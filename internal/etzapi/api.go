...
// NewPublicEthzeroAPI creates a new Ethzero protocol API.
func NewPublicEthzeroAPI(b Backend) *PublicEthzeroAPI {
	return &PublicEthzeroAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicEthzeroAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current Ethzero protocol version this node supports
func (s *PublicEthzeroAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicEthzeroAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19Ethzero Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Ethzero Signed Message:\n%d%s", len(data), data)
...
//
// https://github.com/ethzero/go-ethzero/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with etz_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19Ethzero Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/ethzero/go-ethzero/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Ethzero signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Ethzero blockchain.
// It offers only Methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Ethzero blockchain API.
...
//
// https://github.com/ethzero/wiki/wiki/JSON-RPC#etz_sign
...
// PublicDebugAPI is the collection of Ethzero APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of Ethzero APIs exposed over the private
// debugging endpoint.
...
// Version returns the current ethzero protocol version.
