...
func testGetNodeData(t *testing.T, protocol int) {
	...
	hashes := []common.Hash{}
	for _, key := range pm.chaindb.(*etzdb.MemDatabase).Keys() {
	...
	statedb, _ := etzdb.NewMemDatabase()
...
// Tests that post etz protocol handshake, DAO fork-enabled clients also execute
...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	peer, _ := newTestPeer("peer", etz63, pm, true)
	defer peer.close()
